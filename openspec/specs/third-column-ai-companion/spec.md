...
The AI page SHALL 從 application bundled resources 載入 WebView UI 與 avatar runtime。

#### Scenario: Load AI page offline
- **WHEN** AI tab 被開啟，且沒有外部 local web server 正在執行
- **THEN** WebView 載入 bundled AI interface 與 avatar runtime

#### Scenario: Block unsafe resource paths
- **WHEN** WebView 請求 bundled avatar stage path 之外的 resource
